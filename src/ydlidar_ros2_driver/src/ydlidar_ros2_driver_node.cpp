...
// --- QoS: use RELIABLE so subscribers with Reliable QoS will receive messages ---
  ...
  reliable_qos.deadline(rclcpp::Duration::from_seconds(1.0)); // optional, helps detect problems
  
  auto laser_pub = node->create_publisher<sensor_msgs::msg::LaserScan>("scan", reliable_qos);
  auto pc_pub = node->create_publisher<sensor_msgs::msg::PointCloud>("point_cloud", reliable_qos);
